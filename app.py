...
# Create a combined list of "song name by artist name"
combined_song_artist = songs_data.apply(lambda row: f"{row['name']} by {row['artist']}", axis=1).unique()

# Selectbox for combined song and artist name with autocomplete
selected_song_artist = st.selectbox('Select a song by artist:', options=combined_song_artist)
st.write('You selected:', selected_song_artist)

# Extract song name and artist name from the selected option
if selected_song_artist:
    song_name, artist_name = selected_song_artist.split(' by ')
